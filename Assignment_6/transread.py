...
if __name__ == '__main__':
    ...
    with open('filename3.txt', 'r', encoding='utf-8') as rfile:
    ...
    trans_dict, trans_line = dictmaker(file_names, stopwords)
    importantTerm(trans_dict)
    important_terms = ['transgender', 'genderaffirming', 'healthcare', 'antitrans', 'bill', 'senate', 'states',
                       'federal', 'passed', 'malpractice', 'proposed']
    info_dict = {}
    # This is creating the knowledge base needed for the chatbot being created later
    for term in important_terms:
        info_dict[term] = findSent(term, trans_line)

    pickle.dump(info_dict, open('trans.pickle', 'wb'))
